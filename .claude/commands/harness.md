...
5. **AC는 실행 가능한 커맨드** — "~가 동작해야 한다" 같은 추상적 서술이 아닌 실제 실행 가능한 검증 커맨드를 포함한다. kestrel은 모노레포이므로 해당 step이 건드리는 서비스에 맞는 커맨드를 쓴다:
   - Python(api/·engine/·packages/): `uv run --package <kestrel-api|kestrel-engine> pytest <경로>` 또는 `make test`
   - 프론트(frontend/): `pnpm --dir frontend build` / `pnpm --dir frontend lint`
...
```bash
# 이 step이 건드리는 서비스에 맞춰 작성 (모노레포). 예시:
uv run --package kestrel-api pytest api/tests   # Python 테스트 통과
# 또는 프론트 step이면:
# pnpm --dir frontend build                      # 컴파일/타입 에러 없음
```
...
### F. 마무리 (모든 step 통과 후 자동)

execute.py가 **모든 step을 `completed`로** 끝내면(= 각 step의 AC/테스트가 전부 통과한 상태) 작업을 아래 순서로 마감한다. error/blocked로 끝났으면 마무리하지 않는다 — 테스트가 다 통과한 경우에만 진행한다.

1. **PR 생성**
   ```bash
   gh pr create --base main --head feat-{task-name} --title "{타입}: {요약}" --body "..."
   ```
   본문에 **무엇을·왜** 바꿨는지와 **검증 결과**(각 step의 AC 커맨드와 통과 여부)를 요약한다.

2. **스쿼시 머지**
   ```bash
   gh pr merge --squash --delete-branch
   ```
   - ⚠️ **예외**: 실거래 주문/체결에 직결되는 phase는 자동 머지하지 말고 **사람 확인을 받는다.** (CLAUDE.md CRITICAL — paper 기본·실주문 승인 규칙) 이 경우 PR만 열어두고 멈춘다.

3. **위키 업데이트 (중복 주의)**
   이 task가 추가/변경한 것을 GitHub Wiki에 반영한다.
   - 먼저 기존 위키 페이지를 **읽는다.** 같은 내용이 이미 있으면 새로 만들거나 다시 적지 말고 **갱신**한다.
   - 새 섹션/페이지는 중복이 없을 때만 추가한다. (예: 새 명령어·새 서비스·바뀐 흐름)
   - 위키 repo: `https://github.com/{owner}/{repo}.wiki.git` — 클론 → 수정 → 커밋 → push.

4. **README 업데이트 (필요시에만)**
   사용자에게 보이는 동작·명령어·구성이 바뀌었으면 README를 갱신한다. 바뀐 게 없으면 **건드리지 않는다.**

전제: 이 마무리는 테스트(AC)가 전부 통과했을 때만 의미가 있다. 한 step이라도 error/blocked면 먼저 그것부터 해결한다.
