...
- **packages/broker-client/** — 증권사 추상 인터페이스 + Alpaca 구현. api·engine이 공유하는 공용 패키지. (증권사 교체 가능성을 위해 인터페이스/구현 분리 — docs/ADR.md ADR-008)
- **데이터/인증** — Supabase (PostgreSQL). 세 서비스의 단일 진실 공급원(SSOT).
- **증권사** — Alpaca Open API (REST + WebSocket), 미국 주식. 모의투자(paper) 기본.
...
- **CRITICAL: 증권사 API 호출은 오직 packages/broker-client를 통해서만 한다.** api/나 engine/ 안에서 Alpaca(또는 다른 증권사) REST/WebSocket 엔드포인트를 직접 호출하는(requests/httpx/websocket으로 직접 때리는) 코드를 작성하지 마라. api·engine은 broker-client의 추상 인터페이스에만 의존한다. 클라이언트가 두 벌로 갈라지는 것을 막는다.
...
- 프론트(frontend/)에서 증권사(Alpaca)를 직접 호출하지 마라. 반드시 api/를 경유한다.
