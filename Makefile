.PHONY: help install api engine frontend up down test lock check-token check-price check-daily check-order check-supabase check-balance check-signal-log check-positions check-orders
...
help:
	...
	@echo "check-daily - (수동) 실제 미국 종목 일봉(최근 종가) 조회 확인 (예: make check-daily SYMBOL=TSLA)"
	...
	@echo "check-balance - (수동) 실제 KIS 모의 계좌 예수금·평가자산·보유종목 조회 확인"
...
check-daily:
	uv run --package kestrel-engine python scripts/check_daily.py $(SYMBOL) $(EXCD)
...
check-supabase:
	uv run --package kestrel-engine python scripts/check_supabase.py

check-balance:
	uv run --package kestrel-engine python scripts/check_balance.py

check-signal-log:
	uv run --package kestrel-engine python scripts/check_signal_log.py
